...
def validate_results(data_dir):
    ...
    for d in mock_dirs:
        ...
        truth_files = glob.glob(os.path.join(d, "simulation_truth*.csv"))
        ...
        for suffix, best_suffix, err_suffix in [
            ...
            try:
                ...
                # Check if this strategy was recorded in df_fit
                if f"n_sersic{best_suffix}" not in df_fit.columns:
                    print(f"    Missing columns for {suffix}, skipping.")
                    continue
                
                # Drop rows where this strategy was not run
                df_fit = df_fit.dropna(subset=[f"n_sersic{best_suffix}"])
                
                if len(df_fit) == 0:
                    print(f"    No valid data for {suffix}, skipping.")
                    continue
            ...
            # Rename columns in df_fit: remove specific suffix to align with Truth for easy comparison
            rename_map = {}
            for c in df_fit.columns:
                if c.endswith(best_suffix):
                    rename_map[c] = c[:-len(best_suffix)]
                elif c.endswith(err_suffix):
                    rename_map[c] = c[:-len(err_suffix)] + '_err'
            ...
            plt.savefig(os.path.join(PLOT_DIR, f"validate_{param_name}{suffix}.pdf"))
            plt.close()
            ...
            if available_mps and col_true in merged.columns:
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(PLOT_DIR, f"multipoles_reliability_{param_name}{suffix}.pdf"))
                plt.close()
