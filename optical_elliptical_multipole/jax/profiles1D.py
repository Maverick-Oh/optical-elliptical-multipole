...
from optical_elliptical_multipole.jax.tools import (
...
def _resolve_phi_m(phi_m, theta_m, q):
    ...
    # JAX doesn't like None checks on traced values if successful, but these are likely structural args
    if (phi_m is None) == (theta_m is None):
        raise ValueError("Provide exactly one of phi_m or theta_m.")
    if phi_m is not None:
        return jnp.asarray(phi_m, dtype=float)
    else:
        return jnp.asarray(theta2phi(theta_m, q), dtype=float)
...
def Circular_Multipole_Profile_1D(
    ...
    r = r_vec * (f_ell + (1.0 / jnp.sqrt(q)) * delta)
